...
public class LuceneIndexer implements AutoCloseable { // Implements AutoCloseable
    ...
    private boolean isExcluded(Path p) {
        ...
        for (String g : settings.excludeGlob) {
            try {
                ...
                log.warn("Get path matcher error: glob={}, exception: {}", g, t.getMessage());
            }
            // 兜底：**/xxx/** 的粗略包含判断
    ...
    private boolean isTextMime(Path file) {
        try {
            ...
            log.warn("Probe content type error: {}, exception: {}", file, t.getMessage());
            return false;
        }
    }
    ...
    private boolean looksLikeText(Path file) {
        ...
        try (InputStream is = Files.newInputStream(file, StandardOpenOption.READ)) {
            ...
            log.warn("Looks like text error: {}, exception: {}", file, t.getMessage());
            return false;
        }
    }
}
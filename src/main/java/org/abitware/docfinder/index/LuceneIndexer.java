...
public class LuceneIndexer implements AutoCloseable { // Implements AutoCloseable
    ...
    public void upsertFile(Path file) throws IOException {
        ...
        try {
            ...
            log.warn("Could not extract content or MIME type for file '{}', skipping: {}", file, e.getMessage());
        }
                ...
                Files.walkFileTree(root, new SimpleFileVisitor<Path>() {
                    ...
                    public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) {
                        try {
                            ...
                            log.error("Failed to process file '{}' during indexing, skipping", file, e);
                        }
                        return FileVisitResult.CONTINUE;
                    }
                    ...
                    public FileVisitResult visitFileFailed(Path file, IOException exc) throws IOException {
                        log.error("Failed to visit file '{}', skipping", file, exc);
                        return FileVisitResult.CONTINUE;
                    }
                });
                writer.commit();
    ...
    private boolean isExcluded(Path p) {
        ...
        for (String g : settings.excludeGlob) {
            try {
                ...
                log.warn("Invalid glob pattern '{}' in settings, skipping", g, e);
            }
            // 兜底：**/xxx/** 的粗略包含判断
        ...
        try {
             Future<String> fut = es.submit(() -> {
                try (InputStream is = Files.newInputStream(file, StandardOpenOption.READ)) {
                    ...
                    log.warn("Failed to parse file content for '{}': {}", file, e.getMessage());
                    return "";
                }
            });
            return fut.get(settings.parseTimeoutSec, java.util.concurrent.TimeUnit.SECONDS);
        } catch (Exception timeoutOrOther) {
            log.warn("Content extraction timed out or failed for file '{}': {}", file, timeoutOrOther.getMessage());
            return "";
        } finally {
            es.shutdownNow();
        }
    ...
    private boolean isTextMime(Path file) {
        try {
            ...
            log.warn("Failed to probe MIME type for file '{}': {}", file, e.getMessage());
            return false;
        }
    }
    ...
    private boolean looksLikeText(Path file) {
        ...
        try (InputStream is = Files.newInputStream(file, StandardOpenOption.READ)) {
            ...
            log.warn("Failed to read file for text sniffing '{}': {}", file, e.getMessage());
            return false;
        }
    }
}
...
public class LuceneIndexer implements AutoCloseable { // Implements AutoCloseable
    ...
    public void upsertFile(Path file) throws IOException {
        ...
        try {
            ...
            logger.warn("Could not parse content or get MIME type for file: {}", file, e);
        }
            ...
            try (IndexWriter writer = new IndexWriter(dir, cfg)) {
                Files.walkFileTree(root, new SimpleFileVisitor<Path>() {
                    ...
                    public FileVisitResult visitFileFailed(Path file, IOException exc) throws IOException {
                        logger.error("Could not access file: {}", file, exc);
                        return FileVisitResult.CONTINUE;
                    }
                });
                writer.commit();
    ...
    private boolean isExcluded(Path p) {
        ...
        for (String g : settings.excludeGlob) {
            try {
                ...
                logger.warn("Invalid glob pattern in exclude list: {}", g, e);
            }
            // 兜底：**/xxx/** 的粗略包含判断
        ...
        try {
             Future<String> fut = es.submit(() -> {
                try (InputStream is = Files.newInputStream(file, StandardOpenOption.READ)) {
                    ...
                    logger.warn("Tika parsing failed for file: {}", file, e);
                    return "";
                }
            });
            return fut.get(settings.parseTimeoutSec, java.util.concurrent.TimeUnit.SECONDS);
        } catch (Exception timeoutOrOther) {
            logger.warn("Text extraction timed out or failed for file: {}", file, timeoutOrOther);
            return "";
        } finally {
            es.shutdownNow();
        }
    ...
    private boolean isTextMime(Path file) {
        try {
            ...
            logger.warn("Could not probe MIME type for file: {}", file, e);
            return false;
        }
    }
    ...
    private boolean looksLikeText(Path file) {
        ...
        try (InputStream is = Files.newInputStream(file, StandardOpenOption.READ)) {
            ...
            logger.warn("Could not read file for text-like check: {}", file, e);
            return false;
        }
    }
}